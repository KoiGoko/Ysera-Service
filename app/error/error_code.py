...
class ErrorCode(Enum):
    ...
    RESOURCE_EXHAUSTED = (429, "资源配额不足或达到速率限制。如需了解详情，客户端应该查找 google.rpc.QuotaFailure 错误详细信息。")
    ...
    DEADLINE_EXCEEDED = (504, "超出请求时限。仅当调用者设置的时限比方法的默认时限短（即请求的时限不足以让服务器处理请求）并且请求未在时限范围内完成时，才会发生这种情况。")
    ...
    def __str__(self):
        """
        重写 __str__ 方法，方便打印 ErrorCode 对象。
        """
        return "ErrorCode: code = %s, message = %s" % (self.code, self.message)
