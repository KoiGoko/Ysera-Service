...
def cal_dcf2(source1,DCF_inp,vg1):
    ...
    DCF2=cp.zeros([7,nnuclide,nts])    # 用于保存核素合并的DCF
    for k in range(7):
        for i in range(nts):
            if k==0 or k==6:
...
def cal_dose_1(xq1,DCF_input,flag=None):
    ...
    Dose1=cp.zeros([no,nts,ny,nx]).astype(cp.float32)

    # 将每种类型的剂量转换因子进行维度扩充，变成三维数组
    for i in range(no):
        DCFs[i]=cp.expand_dims(DCFs[i],axis=(1,2))
        Dose1[i,:,:,:]=xq1*DCFs[i]
...
if __name__ == '__main__':
    ...
    sname=list(source.keys())
    source0=cp.array(source[sname[0]].iloc[:,1:]).astype(cp.float32)
    nts=cp.shape(source0)[1]    # 源项对应的小时数，num_time_source
    ...
    (nta,ny,nx)=cp.shape(xqa)
    nt=int((nta-nts)/3) # 抽样后计算次数

    for i in range(len(source)):
        #1 获取源项
        source1=cp.array(source[sname[i]].iloc[:,1:]).astype(cp.float32)
        ...
        #3 初始化剂量数组
        data1=cp.zeros([len(calDCF),nt,ny,nx],dtype='float32')
        ...
        #5  将一个剂量结果整理并输出
        data2=np.float32(cp.asnumpy(data1))
        print(data2[0,0,:3,:3])
        save_dose(data2,os.path.join(dir_out, outfile1))
