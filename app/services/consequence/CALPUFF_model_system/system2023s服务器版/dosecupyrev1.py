...
def read_DCF(dir_DCF):
    ...
    vg=cp.array(myDCF['Vg(m/s)']).astype(np.float32)
    lamta=cp.array(myDCF['lamta(1/h)']).astype(np.float32)
    DCFinp=cp.array(myDCF.loc[:,['air','ground','inhaled','thyroid',
                             'red_marr','lungs','skin']]).astype(cp.float32)
...
def cal_dcf2(source1,DCF_inp,vg1):
    ...
    '''
    br=2.3e-4;
    (nnuclide,nts) = cp.shape(source1)

    DCF2_air=cp.zeros(cp.shape(source1))    # 用于保存核素合并的DCF
    DCF2_grd=cp.zeros(cp.shape(source1))
    DCF2_in =cp.zeros(cp.shape(source1))
    DCF2_th =cp.zeros(cp.shape(source1))

    DCF2_red =cp.zeros(cp.shape(source1))
    DCF2_lung=cp.zeros(cp.shape(source1))
    DCF2_skin =cp.zeros(cp.shape(source1))
...
def read_xq(dir_xq):
    ...
    """
    #dir_xq=r'J:\mx\dispersion\xq\*.nc'
    with xr.open_mfdataset(dir_xq,concat_dim='t',combine='nested') as ds:
        xq=ds['KR-85'].data
        xq=cp.array(xq)
...
def dose_cal(source1,xq,xqtime,xygrid,DCF_eff,DCF_th,dir_out,outfile):
    ...
    '''
    s=time.time()
    nts=cp.shape(source1)[1]     # 源项对应的小时数，num_time_source
    dt=3
    (nty,ny,nx)=cp.shape(xq)    # 整年弥散因子数据
    nt=int((nty-nts)/dt) # 抽样后计算次数

    # 一次抽样的剂量计算数组
    Dose1_eff=cp.zeros([nts,ny,nx]).astype(cp.float32)
    Dose1_th=cp.zeros([nts,ny,nx]).astype(cp.float32)

    # 全年抽样的剂量数组
    Dose_eff1= cp.zeros([nt,ny,nx]).astype(cp.float32)
    Dose_th1 = cp.zeros([nt,ny,nx]).astype(cp.float32)


    # 将剂量转换因子进行扩充，变成三维数组

    DCF1_eff=cp.expand_dims(DCF_eff,axis=(1,2))
    DCF1_eff=cp.tile(DCF1_eff,(1,ny,nx))

    DCF1_th=cp.expand_dims(DCF_th,axis=(1,2))
    DCF1_th=cp.tile(DCF1_th,(1,ny,nx))


    for k in cp.arange(nt):
        kk=k*dt
        xq1=xq[kk:nts+kk,:,:]
    ...
    # 将计算结果输出到文件
    Dose_eff1 = np.float32(cp.asnumpy(Dose_eff1))
    Dose_th1  = np.float32(cp.asnumpy(Dose_th1))
...
def acutedose_cal(source1,xq,xqtime,xygrid,DCF_red,DCF_lung,DCF_skin,dir_out,outfile):
    ...
    # 一次抽样的剂量计算数组
    Dose1_red=cp.zeros([nts,ny,nx]).astype(cp.float32)
    Dose1_lung=cp.zeros([nts,ny,nx]).astype(cp.float32)
    Dose1_skin=cp.zeros([nts,ny,nx]).astype(cp.float32)

    # 全年抽样的剂量数组

    Dose_red1= cp.zeros([nt,ny,nx]).astype(cp.float32)
    Dose_lung1 = cp.zeros([nt,ny,nx]).astype(cp.float32)
    Dose_skin1= cp.zeros([nt,ny,nx]).astype(cp.float32)

    # 将剂量转换因子进行扩充，变成三维数组

    DCF1_red=cp.expand_dims(DCF_red,axis=(1,2))
    DCF1_red=cp.tile(DCF1_red,(1,ny,nx))

    DCF1_lung=cp.expand_dims(DCF_lung,axis=(1,2))
    DCF1_lung=cp.tile(DCF1_lung,(1,ny,nx))

    DCF1_skin=cp.expand_dims(DCF_skin,axis=(1,2))
    DCF1_skin=cp.tile(DCF1_skin,(1,ny,nx))
    ...
    # 将计算结果输出到文件
    Dose_red1 = np.float32(cp.asnumpy(Dose_red1))
    Dose_lung1  = np.float32(cp.asnumpy(Dose_lung1))
    Dose_skin1  = np.float32(cp.asnumpy(Dose_skin1))
